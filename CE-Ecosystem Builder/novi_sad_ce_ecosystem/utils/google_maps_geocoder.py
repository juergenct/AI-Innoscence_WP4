...
class GoogleMapsGeocoder:
    ...
    def _geocode_with_retry(self, query: str, region: str = "rs") -> Tuple[Optional[float], Optional[float]]:
        """
        Geocode a query using Google Maps API with retry logic.

        Args:
            query: Address or place name to geocode
            region: Region bias (default: "rs" for Serbia)
    ...
    def geocode_novi_sad(
        ...
        """
        Geocode an address in Novi Sad with proper formatting.
        ...
        # Format address with Novi Sad, Serbia for better results
        formatted_address = f"{address.strip()}, Novi Sad, Serbia"

        return self.geocode_address(formatted_address, region="rs")
