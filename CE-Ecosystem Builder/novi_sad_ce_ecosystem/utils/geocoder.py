...
class Geocoder:
    def __init__(
        self,
        user_agent: str = "AI-Innoscence_NoviSad_CE_Mapper/1.0",
    ...
    def _geocode_with_retry(self, query: str, country_codes: str = "rs") -> Tuple[Optional[float], Optional[float]]:
    ...
    def geocode_novi_sad(
        ...
        """
        Geocode an address in Novi Sad with fallback strategies.
        ...
        if address and address.strip():
            query = f"{address.strip()}, Novi Sad, Serbia"
        ...
        # Strategy 2.5: Try URL domain name + Novi Sad (before generic Novi Sad fallback)
        if use_fallback and url:
            try:
                ...
                if domain_name and domain_name.lower() not in ['unknown', 'na', 'n/a', 'localhost', '127']:
                    query = f"{domain_name}, Novi Sad, Serbia"
        ...
        # Strategy 3: Last resort - Novi Sad city center (for incomplete extractions)
        if use_fallback:
            ...
            if query in self.cache:
                cached = self.cache[query]
                if cached[0] is not None and cached[1] is not None:
                    self.logger.debug("Using Novi Sad city center fallback")
                    return cached
