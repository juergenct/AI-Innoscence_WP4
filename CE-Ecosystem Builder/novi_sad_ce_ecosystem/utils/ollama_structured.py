...
class VerificationResult(BaseModel):
    """Pydantic model for Novi Sad/CE verification."""
    is_novi_sad_based: bool = Field(description="Is entity based in Novi Sad, Serbia?")
    novi_sad_confidence: float = Field(ge=0.0, le=1.0, description="Confidence score (0-1 scale)")
    novi_sad_evidence: str = Field(description="Evidence found for Novi Sad location")
    ...
    @field_validator('novi_sad_confidence', 'ce_confidence', mode='before')
    @classmethod
...
class ExtractionResult(BaseModel):
    ...
    mentioned_partners: list[dict] = Field(
        default_factory=list,
        description='Partners as list of dicts with keys: name, url (optional), context. Example: [{"name": "Hamburg Recycling", "url": "https://...", "context": "Partnership for waste processing"}]'
    )
    ...
    address: str = Field(default="", description="Complete physical address in Novi Sad with postal code")
...
class EcosystemGapAnalysisResult(BaseModel):
    ...
    geographic_gaps: list[str] = Field(
        default_factory=list,
        description="Geographic areas within Novi Sad that lack CE actors"
    )
...
def call_ollama_chat(
    ...
    try:
        # Increased timeout to 600 seconds (10 min) for 32B model and complex Serbian/Cyrillic text processing
        ...
        # Enhanced error reporting for truncation issues
        error_msg = str(e)
        if "EOF while parsing" in error_msg or "json_invalid" in error_msg:
            raise RuntimeError(f"Ollama API call failed (JSON truncation - response may be too long): {e}")
