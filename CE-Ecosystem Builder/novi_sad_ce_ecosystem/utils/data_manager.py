...
class DataManager:
    ...
    def _init_db(self) -> None:
        ...
        cur.execute(
            '''CREATE TABLE IF NOT EXISTS verification_results (
                ...
                url TEXT UNIQUE,
                is_novi_sad_based INTEGER,
                novi_sad_confidence REAL,
        ...
        cur.execute('CREATE INDEX IF NOT EXISTS idx_verification_novi_sad ON verification_results(is_novi_sad_based);')
    ...
    def save_verification(self, items: Iterable[Dict[str, Any]]) -> None:
        ...
        for it in items:
            cur.execute(
                ...
                (url, is_novi_sad_based, novi_sad_confidence, is_ce_related, ce_confidence, reasoning, should_extract, input_category)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(url) DO UPDATE SET
                    is_novi_sad_based=excluded.is_novi_sad_based,
                    novi_sad_confidence=excluded.novi_sad_confidence,
                ...
                ;''', (
                    str(it.get('url') or ""),
                    int(bool(it.get('is_novi_sad_based'))),
                    float(it.get('novi_sad_confidence') or 0.0),
    ...
    def get_geocoded_urls(self) -> set[str]:
        """Get set of URLs that have already been geocoded (have latitude and longitude).

        Returns:
            Set of URLs that have geocoding information.
        """
        cur = self._conn.cursor()
    ...
    def get_all_entity_profiles(self) -> list[Dict[str, Any]]:
        """Get all entity profiles from the database.

        Returns:
            List of dictionaries containing all entity profile data.
        """
        cur = self._conn.cursor()
        ...
        for row in rows:
            ...
            # Parse JSON fields
            for field in ['contact_persons', 'emails', 'phone_numbers', 'ce_activities',
    ...
    def update_coordinates(self, url: str, latitude: float, longitude: float) -> None:
        """Update latitude and longitude for a specific entity profile.

        Args:
            url: The URL of the entity to update
