...
def prepare_input_urls(base_dir: Path) -> Path:
    ...
    # Final fallback: User must provide entity_urls.json
    print("ERROR: No input file found!")
    print("Please create data/input/entity_urls.json with entity URLs")
    print("Or provide config/entity_sources.json with source data")
    raise FileNotFoundError("Input file data/input/entity_urls.json is required")


def main() -> None:
    base_dir = Path(__file__).resolve().parent
    print("Cahul CE Ecosystem Mapper")
    print("-" * 50)
