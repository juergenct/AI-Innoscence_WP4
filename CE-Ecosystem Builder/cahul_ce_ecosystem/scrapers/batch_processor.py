...
from cahul_ce_ecosystem.models.schemas import (
    VerificationResultDict,
    EntityProfileDict,
    RelationshipDict,
    EcosystemInsightDict,
)
from cahul_ce_ecosystem.scrapers.verification_scraper import VerificationScraper
from cahul_ce_ecosystem.scrapers.extraction_scraper import ExtractionScraper
from cahul_ce_ecosystem.scrapers.relationship_analyzer import RelationshipAnalyzer
from cahul_ce_ecosystem.utils.logging_setup import setup_logging
from cahul_ce_ecosystem.utils.geocoder import Geocoder
from cahul_ce_ecosystem.utils.url_utils import extract_domain
from cahul_ce_ecosystem.utils.rate_limiter import DomainRateLimiter
from cahul_ce_ecosystem.utils.data_manager import DataManager
from cahul_ce_ecosystem.utils.suppress_warnings import suppress_verbose_output
...
class BatchProcessor:
    def __init__(self, input_file: str | Path, config_path: str | Path | None = None, max_workers: int = 8):
        ...
        self.logger = setup_logging(self.input_file.parent.parent.parent / 'logs' / 'scraping_errors_cahul.log', console_level=logging.ERROR)
    ...
    def run_extraction(self, verified_entities: List[VerificationResultDict]) -> List[EntityProfileDict]:
        ...
        already_extracted = self.dm.get_extracted_urls()
        ...
        for entity in to_extract:
            ...
            if self.extraction_scraper.cache.get(cache_key):
                cache_extracted.add(entity['url'])

        # Combine database and cache results
        all_extracted = already_extracted | cache_extracted

        # Filter to only process URLs not yet extracted
        original_count = len(to_extract)
        to_extract = [e for e in to_extract if e['url'] not in all_extracted]

        print(f"✅ Resume checkpoint: {len(all_extracted)} already extracted")
        print(f"   - {len(already_extracted)} in database")
        print(f"   - {len(cache_extracted)} in cache only")
        ...
        new_results = asyncio.run(
            self._run_extraction_async(
                to_extract,
                save_path,
                already_completed=len(already_extracted),
        ...
        if save_path.exists() and len(already_extracted) > 0:
            try:
                existing_data = json.loads(save_path.read_text(encoding='utf-8'))
    ...
    def geocode_entities(self, profiles: List[EntityProfileDict]) -> List[EntityProfileDict]:
        ...
        # Define Cahul city center coordinates (default fallback)
        CAHUL_CENTER_LAT = 45.908298
        CAHUL_CENTER_LON = 28.194431

        # Count entities still needing geocoding
        # Entities need geocoding if:
        # 1. They have no coordinates (None), OR
        # 2. They have Cahul center coordinates (will retry with name/URL fallback)
        to_geocode = [
            p for p in profiles
            if (p.get('latitude') is None or p.get('longitude') is None) or
               (abs(p.get('latitude', 0) - CAHUL_CENTER_LAT) < 0.000001 and
                abs(p.get('longitude', 0) - CAHUL_CENTER_LON) < 0.000001)
        ]
        ...
        for idx, p in enumerate(tqdm(profiles, desc="Geocoding", unit="entity"), 1):
            # Check if entity needs geocoding (None coords OR Cahul center)
            needs_geocoding = (
            ...
            if needs_geocoding:
                ...
                lat, lon = geocoder.geocode_cahul(
        ...
        # VALIDATION: Report on entities still using Cahul center coordinates
        cahul_center_entities = [
            p for p in profiles
            if abs(p.get('latitude', 0) - CAHUL_CENTER_LAT) < 0.000001 and
               abs(p.get('longitude', 0) - CAHUL_CENTER_LON) < 0.000001
        ]

        cahul_with_address = [
            p for p in cahul_center_entities
            if p.get('address') and
               str(p.get('address')).strip() and
               str(p.get('address')).strip().lower() not in ['unknown', 'na', 'n/a', 'none']
        ]

        cahul_without_address = [
            p for p in cahul_center_entities
        ...
        print(f"\n📍 Cahul Center Coordinate Analysis:")
        print(f"   - Total entities with Cahul center coords: {len(cahul_center_entities)}")
        print(f"   - With valid addresses (needs Google Maps): {len(cahul_with_address)}")
        print(f"   - Without addresses (expected fallback): {len(cahul_without_address)}")

        # STAGE 2: Google Maps Fallback (if enabled and needed)
        if cahul_with_address:
            print(f"\n⚠️  Found {len(cahul_with_address)} entities with addresses but Cahul center coordinates")
            ...
            if enable_google_fallback and google_api_key:
                ...
                print(f"{'='*80}")
                print(f"💰 Estimated cost: ${len(cahul_with_address) * 0.005:.2f} USD")

                # Initialize Google Maps geocoder
                from cahul_ce_ecosystem.utils.google_maps_geocoder import GoogleMapsGeocoder
                ...
                for p in tqdm(cahul_with_address, desc="Google Maps Geocoding", unit="entity"):
                    lat, lon = google_geocoder.geocode_cahul(p.get('address'))

                    if lat is not None and lon is not None:
                        # Verify it's not the Cahul center
                        if not (abs(lat - CAHUL_CENTER_LAT) < 0.000001 and abs(lon - CAHUL_CENTER_LON) < 0.000001):
                            p['latitude'] = lat
                            p['longitude'] = lon
                ...
                print(f"   - {google_geocoded_count}/{len(cahul_with_address)} successfully geocoded")
    ...
    def run_pipeline(self) -> None:
        print("\n🔍 Stage 1: Verifying Cahul + CE relevance...")
        self.logger.info("Stage 1: Verifying Cahul + CE relevance...")
