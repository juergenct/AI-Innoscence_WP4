...
def analyze_impressum(home_url: str) -> Dict[str, object]:
    ...
    if not url:
        return {"found": False, "cahul_confidence": 0.0, "evidence": ""}
    ...
    postal_hit = re.search(r"MD-[0-9]{4}", text) is not None
    name_hit = any(n in lower for n in [
        "cahul", "congaz", "giurgiulești", "giurgiulesti", "raionul cahul", "moldova",
    ])
    phone_hit = "+373" in lower
