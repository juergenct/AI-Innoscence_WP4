    ...
    def _init_db(self) -> None:
        ...
        cur.execute(
            '''CREATE TABLE IF NOT EXISTS verification_results (
                ...
                url TEXT UNIQUE,
                is_cahul_based INTEGER,
                cahul_confidence REAL,
        ...
        cur.execute('CREATE INDEX IF NOT EXISTS idx_verification_cahul ON verification_results(is_cahul_based);')
    ...
    def save_verification(self, items: Iterable[Dict[str, Any]]) -> None:
        ...
        for it in items:
            cur.execute(
                ...
                (url, is_cahul_based, cahul_confidence, is_ce_related, ce_confidence, reasoning, should_extract, input_category)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(url) DO UPDATE SET
                    is_cahul_based=excluded.is_cahul_based,
                    cahul_confidence=excluded.cahul_confidence,
                ...
                ;''', (
                    str(it.get('url') or ""),
                    int(bool(it.get('is_cahul_based'))),
                    float(it.get('cahul_confidence') or 0.0),
