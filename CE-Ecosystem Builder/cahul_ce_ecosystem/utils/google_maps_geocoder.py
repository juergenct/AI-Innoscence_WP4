...
class GoogleMapsGeocoder:
    ...
    def _geocode_with_retry(self, query: str, region: str = "md") -> Tuple[Optional[float], Optional[float]]:
        """
        Geocode a query using Google Maps API with retry logic.

        Args:
            query: Address or place name to geocode
            region: Region bias (default: "md" for Moldova)
    ...
    def geocode_cahul(
        ...
        """
        Geocode an address in Cahul with proper formatting.
        ...
        # Format address with Cahul, Moldova for better results
        formatted_address = f"{address.strip()}, Cahul, Moldova"

        return self.geocode_address(formatted_address, region="md")
