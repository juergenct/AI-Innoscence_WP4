...
class Geocoder:
    def __init__(
        self,
        user_agent: str = "AI-Innoscence_Hamburg_CE_Mapper/1.0",
    ...
    def _geocode_with_retry(self, query: str, country_codes: str = "de") -> Tuple[Optional[float], Optional[float]]:
    ...
    def geocode_hamburg(
        ...
        """
        Geocode an address in Hamburg with fallback strategies.
        ...
        if address and address.strip():
            query = f"{address.strip()}, Hamburg, Germany"
        ...
        # Strategy 2.5: Try URL domain name + Hamburg (before generic Hamburg fallback)
        if use_fallback and url:
            try:
                ...
                if domain_name and domain_name.lower() not in ['unknown', 'na', 'n/a', 'localhost', '127']:
                    query = f"{domain_name}, Hamburg, Germany"
        ...
        # Strategy 3: Last resort - Hamburg city center (for incomplete extractions)
        if use_fallback:
            ...
            if query in self.cache:
                cached = self.cache[query]
                if cached[0] is not None and cached[1] is not None:
                    self.logger.debug("Using Hamburg city center fallback")
                    return cached
