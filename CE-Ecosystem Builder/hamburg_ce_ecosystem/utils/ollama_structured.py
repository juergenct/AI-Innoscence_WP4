...
class VerificationResult(BaseModel):
    ...
    hamburg_confidence: float = Field(ge=0.0, le=1.0, description="Confidence score (0-1 scale)")
    ...
    @field_validator('hamburg_confidence', 'ce_confidence', mode='before')
    @classmethod
...
class EcosystemGapAnalysisResult(BaseModel):
    ...
    geographic_gaps: list[str] = Field(
        default_factory=list,
        description="Geographic areas within Hamburg that lack CE actors"
    )
